...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-blue-400">Vrienden</h1>
        </div>

        <!-- Friend Search -->
        <div class="bg-gray-800 rounded-lg shadow p-6 mb-6">
            <h2 class="text-xl font-bold mb-4 text-blue-400">Vrienden Zoeken</h2>
            ...
            @if(isset($searchResults))
                <div class="mt-4">
                    <h3 class="font-medium mb-2 text-gray-300">Zoekresultaten:</h3>
                    @forelse($searchResults as $user)
                        <div class="flex items-center justify-between py-2 border-b border-gray-700">
                            <div class="flex items-center">
                                ...
                                @else
                                    <div class="w-8 h-8 bg-gray-600 rounded-full mr-3 flex items-center justify-center text-white">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div class="bg-gray-800 rounded-lg shadow p-6">
                <h2 class="text-xl font-bold mb-4 text-orange-400">Vriendschapsverzoeken</h2>
                @forelse($pendingRequests as $request)
                    <div class="flex items-center justify-between py-3 border-b border-gray-700 last:border-b-0">
                        <div class="flex items-center">
                            ...
                            @else
                                <div class="w-10 h-10 bg-gray-600 rounded-full mr-3 flex items-center justify-center text-white">
                                    {{ substr($request->requester->username, 0, 1) }}
                                </div>
                            @endif
                            <div>
                                <span class="font-medium text-gray-300">{{ $request->requester->username }}</span>
                                <p class="text-sm text-gray-400">{{ $request->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
            ...
            <!-- Friends List -->
            <div class="bg-gray-800 rounded-lg shadow p-6">
                <h2 class="text-xl font-bold mb-4 text-green-400">Mijn Vrienden</h2>
                @forelse($friends as $friend)
                    <div class="flex items-center justify-between py-3 border-b border-gray-700 last:border-b-0">
                        <div class="flex items-center">
                            ...
                            @endif
                            <div>
                                <span class="font-medium text-gray-300">{{ $friend->username }}</span>
